...
const VideoCallDetection = ({ onBack }) => {
    ...
    const captureAndAnalyze = useCallback(() => {
        ...
        if (!video || !video.videoWidth || analyzingRef.current) return;
        ...
        canvas.toBlob(async (blob) => {
            ...
            try {
                ...
                formData.append('image', new File([blob], 'frame.png', { type: 'image/png' }));
                ...
                setError('Analysis failed — check backend connection at port 8000');
            } finally {
                analyzingRef.current = false;
                setIsAnalyzing(false);
            }
    ...
    const alertLevel = !isCapturing ? 'idle'
        : result?.overall === 'deepfake' ? 'danger'
    ...
    return (
        <div className="min-h-screen bg-slate-900 text-white flex flex-col">
            <nav className="p-4 border-b border-slate-800 flex items-center justify-between">
                ...
                <button
                    onClick={onBack}
                    className="flex items-center gap-2 px-4 py-2 hover:bg-slate-800 rounded-lg transition text-sm"
                >
                ...
                {!isCapturing && (
                    <div className="p-5 bg-slate-800 rounded-2xl border border-slate-700">
                        ...
                        <ol className="space-y-1.5 text-slate-300 text-sm list-decimal list-inside">
                            ...
                            <li>The system captures a frame every 3 seconds and runs deepfake detection</li>
                            <li>A red alert appears immediately if a deepfake face is detected</li>
                        </ol>
                        <p className="text-xs text-slate-500 mt-3">
                            Compatible with Google Meet, Zoom (web), Microsoft Teams, and any browser-based video call
                        </p>
                    </div>
                )}
                ...
                {isCapturing && (
                    <div className={`flex items-center justify-between p-4 rounded-xl border transition-all duration-500 ${alertStyles[alertLevel]}`}>
                        <div className="flex items-center gap-3">
                            <span className={`w-3 h-3 rounded-full ${alertLevel === 'danger' ? 'bg-red-500' : alertLevel === 'safe' ? 'bg-green-500' : 'bg-blue-500'} animate-pulse`} />
                        ...
                        <div className="text-xs text-slate-400">
                            {history.length} frames analyzed · {deepfakeCount} deepfake(s) flagged
                        </div>
                    </div>
                )}
                ...
                <div className="relative bg-black rounded-2xl overflow-hidden border border-slate-700" style={{ aspectRatio: '16/9' }}>
                    ...
                    {!isCapturing && (
                        <div className="absolute inset-0 flex flex-col items-center justify-center bg-slate-900 text-slate-500">
                            ...
                            <p className="text-sm">Screen preview will appear here after you start capture</p>
                        </div>
                    )}
                    {isCapturing && (
                    ...
                    {result && isCapturing && (
                        <div className={`absolute bottom-3 right-3 flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-bold backdrop-blur-sm ${
                            result.overall === 'real'
                            ...
                            {result.overall === 'real' ? 'Real' : 'Deepfake'} · {result.deepfake?.confidence}%
                        </div>
                    )}
                </div>
                ...
                <div className="flex gap-3 items-center">
                    {!isCapturing ? (
                        <button
                            ...
                            className="flex items-center gap-2 bg-blue-600 hover:bg-blue-700 px-6 py-3 rounded-full font-bold transition shadow-lg"
                        >
                        ...
                        <button
                            onClick={stopCapture}
                            className="flex items-center gap-2 bg-red-600 hover:bg-red-700 px-6 py-3 rounded-full font-bold transition shadow-lg"
                        >
                            <Square className="w-4 h-4" />
                            Stop Capture
                        </button>
                    )}
                    <span className="text-xs text-slate-500">Analyzes every {CAPTURE_INTERVAL_MS / 1000}s</span>
                </div>
                            ...
                            {history.map((item, i) => (
                                ...
                                >
                                    <div className="flex items-center gap-2">
                                        ...
                                        <span className={`font-semibold capitalize ${item.overall === 'real' ? 'text-green-300' : 'text-red-300'}`}>
                                            {item.overall}
                                        </span>
                                        <span className="text-slate-400">· {item.deepfake?.confidence}% confidence</span>
                                    </div>
